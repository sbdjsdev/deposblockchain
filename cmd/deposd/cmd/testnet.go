...
Example:
	...
	cmd.Flags().String(flagNodeDaemonHome, "deposd", "Home directory of the node's daemon configuration")
...
const dockerComposeDefinition = `# DeposNetwork Testnet
version: '3.1'
services:{{range $node := .Nodes }}
	{{ $node.Name }}:
		image: trdtech/deposnetwork:{{ $.Tag }}
		restart: always
		ports:
