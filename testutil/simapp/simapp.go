...
import (
	...
	stargazeapp "github.com/trdtech/deposnetwork/app"
)
...
func New(dir string) *stargazeapp.App {
	...
	a := stargazeapp.NewDeposNetworkApp(logger, db, nil, true, map[int64]bool{}, dir, 0, encoding,
...
func setup(withGenesis bool, invCheckPeriod uint, dir string) (*stargazeapp.App, stargazeapp.GenesisState) {
	db := tmdb.NewMemDB()
	encoding := cosmoscmd.MakeEncodingConfig(stargazeapp.ModuleBasics)
	a := stargazeapp.NewDeposNetworkApp(log.NewNopLogger(), db, nil, true,
